...
const TimeTracker = () => {
  ...
  const handleTabChange = (newTab) => {
    ...
    const timeout = setTimeout(() => {
      setActiveTab(newTab);
      try {
        // update URL so tab is linkable
        navigate(`${location.pathname}?tab=${encodeURIComponent(newTab)}`, { replace: true });
      } catch (e) {
  ...
  // Read tab from URL (?tab=...) so the tab is directly addressable
  useEffect(() => {
    try {
      ...
      {activeTab === 'brands' && (
        <div className="brands-tab-content" style={{ width: '100%' }}>
          <div style={{ marginBottom: 12 }}>
            <h1 style={{ margin: '0 0 12px 0' }}>BIM Tracker</h1>
          </div>
          <div style={{ width: '100%' }}>
