  ...
  return (
    <div>
      ...
      <div className="global-organization-manager">
          ...
          <>
            <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
              <div>
                ...
                {/* Summary: organizations per brand */}
                <div style={{ marginTop: '0.5rem', color: '#374151', fontSize: '0.95rem' }}>
                  <strong style={{ display: 'block', marginBottom: '0.25rem' }}>Organizations per Brand</strong>
                  <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                    {brands.map(brand => {
                      const orgCount = organizations.filter(org => {
                        const orgBrand = org.brand || org.brandId || org.brand_id || '';
                        if (!orgBrand) return false;
                        const bName = (brand.name || '').toString().toLowerCase();
                        return String(orgBrand).toLowerCase() === String(brand.id).toLowerCase() || String(orgBrand).toLowerCase() === bName;
                      }).length;
                      return (
                        <span key={brand.id} style={{ background: '#f3f4f6', padding: '0.25rem 0.5rem', borderRadius: '12px', color: '#374151', fontSize: '0.9rem' }}>
                          {brand.name || brand.id}: {orgCount}
                        </span>
                      );
                    })}
                  </div>
        ...
        {showBrandModal && (
          <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div className="modal-content" style={{ background: 'white', borderRadius: '8px', padding: '2rem', maxWidth: '600px', width: '92%', maxHeight: '90vh', overflow: 'auto' }}>
              <div className="modal-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                <h3>{editingBrand ? 'Edit Brand' : 'Add New Brand'}</h3>
                <button onClick={closeBrandModal} style={{ background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer' }}>×</button>
              </div>
              ...
              <div className="form-group" style={{ marginBottom: '1rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
                <div style={{ width: '120px', height: '80px', borderRadius: '8px', overflow: 'hidden', background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {brandImagePreview || newBrand.imageUrl ? (
                    <img src={brandImagePreview || newBrand.imageUrl} alt="brand" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  ) : (
                    <div style={{ color: '#6b7280', fontSize: '0.9rem' }}>No Image</div>
                  )}
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: '0.9rem', marginBottom: '0.25rem' }}>Upload Image</label>
                  <input type="file" accept="image/*" onChange={handleBrandImageChange} />
                  <p style={{ margin: '0.5rem 0 0', color: '#6b7280', fontSize: '0.85rem' }}>Optional - used for brand listings and logos.</p>
                </div>
              </div>

              <div className="form-group" style={{ marginBottom: '1rem' }}>
                <label style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
                  <input type="checkbox" checked={editingBrand ? (editingBrand.isActive !== false) : true} onChange={e => {
                    // update local newBrand state to reflect isActive
                    setNewBrand(prev => ({ ...prev, isActive: e.target.checked }));
                  }} />
                  <span style={{ marginLeft: '0.25rem' }}>Active</span>
                </label>
              </div>

              <div className="modal-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', marginTop: '1rem' }}>
                <button onClick={closeBrandModal} style={{ background: '#6b7280', color: 'white', border: 'none', padding: '0.5rem 1rem', borderRadius: '4px', cursor: 'pointer' }}>Cancel</button>
                <button onClick={saveBrand} style={{ background: '#10b981', color: 'white', border: 'none', padding: '0.5rem 1rem', borderRadius: '4px', cursor: 'pointer' }}>{editingBrand ? 'Update' : 'Create'} Brand</button>
              </div>
            </div>
          </div>
        )}
