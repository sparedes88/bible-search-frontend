...
const BalanceManager = () => {
  ...
  // Initialize Stripe only when the recharge form is shown to avoid loading stripe.js on every page
  useEffect(() => {
    let mounted = true;
    const initStripe = async () => {
      const publicKey = process.env.REACT_APP_STRIPE_PUBLIC_KEY || "pk_live_lFVwMl9P5iRRyJEe3UFIQxeA";
      if (!publicKey || publicKey === 'your_stripe_public_key') {
        // Do not attempt to load Stripe if key is missing or clearly a placeholder
        setStripeError('Stripe public key not configured');
        return;
      }

      try {
        const p = loadStripe(publicKey);
        // await resolution to catch load failures early
        await p.catch(err => { throw err; });
        if (mounted) setStripePromise(p);
      } catch (err) {
        console.error('Failed to initialize Stripe:', err);
        if (mounted) setStripeError('Failed to load Stripe.js');
      }
    };

    if (showRechargeForm && !stripePromise && !stripeError) {
      initStripe();
    }

    return () => { mounted = false; };
  }, [showRechargeForm, stripePromise, stripeError]);
      ...
      <div className="balance-manager-container">
        ...
        {showRechargeForm && (
          <div className="recharge-section">
            ...
            {stripeError ? (
              <div className="loading-stripe-element">Payment system unavailable</div>
            ) : !stripePromise ? (
              <div className="loading-stripe-element">Loading payment form...</div>
            ) : (
