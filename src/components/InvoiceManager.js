...
const InvoiceManager = () => {
  ...
  // Initialize Stripe only when a payment is being made to avoid loading stripe.js globally
  useEffect(() => {
    let mounted = true;
    const initStripe = async () => {
      ...
      if (!publicKey || publicKey === 'your_stripe_public_key') {
        setStripeInitError('Stripe public key not configured');
        return;
      }
      ...
      try {
        ...
        if (mounted) setStripeInitError('Failed to load Stripe.js');
      }
    };

    if (payingInvoice && !stripePromise && !stripeInitError) {
      initStripe();
    }

    return () => { mounted = false; };
  }, [payingInvoice, stripePromise, stripeInitError]);
